...
void printf_formateado (SharedData *shared_data){
    // Encabezados
    ...
    char *token = strtok(shared_data->result, "\t\n");
    int field = 0;

    while (token != NULL) {
        switch (field % 9) {
            case 0: printf("%-10s ", token); break;       // tconst
            case 1: printf("%-10s ", token); break;       // titleType
            case 2: printf("%-25s ", token); break;       // primaryTitle
            case 3: printf("%-25s ", token); break;       // originalTitle
            case 4: printf("%-6s ", token); break;        // isAdult
            case 5: printf("%-6s ", token); break;        // startYear
            case 6: printf("%-6s ", token); break;        // endYear
            case 7: printf("%-6s ", token); break;        // runtimeMinutes
            case 8: printf("%-20s\n", token); break;      // genres
        }
        token = strtok(NULL, "\t\n");
        field++;
